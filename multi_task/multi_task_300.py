...
import numpy as np
from chainercv.links.model.ssd.ssd_vgg16 import VGG16Extractor300, Multibox
...
class Multi_task_VGG16Extractor300(VGG16Extractor300):
    def __init__(self, detection=False, segmentation=False, attention=False):
        ...
        with self.init_scope():
            if self.detection:
                self.deconv_det_1 = L.Deconvolution2D(256, 256, ksize=3, stride=1, pad=0, **init)  ##
                ...
                self.deconv_det_2 = L.Deconvolution2D(256, 256, ksize=3, stride=1, pad=0, **init)  ##
                ...
                self.deconv_det_3 = L.Deconvolution2D(256, 256, ksize=4, stride=2, pad=1, **init)  ##

                self.deconv_conv_det_3 = L.Convolution2D(256, 512, ksize=3, stride=1, pad=1, **init)  #
                self.skip_conv_det_3 = L.Convolution2D(512, 512, ksize=3, stride=1, pad=1, **init)  #

                self.deconv_det_4 = L.Deconvolution2D(512, 256, ksize=3, stride=2, pad=1, **init)  ##

                self.deconv_conv_det_4 = L.Convolution2D(256, 1024, ksize=3, stride=1, pad=1, **init)  #
                self.skip_conv_det_4 = L.Convolution2D(1024, 1024, ksize=3, stride=1, pad=1, **init)  #

                self.deconv_det_5 = L.Deconvolution2D(1024, 256, ksize=4, stride=2, pad=1, **init)

                self.deconv_conv_det_5 = L.Convolution2D(256, 512, ksize=3, stride=1, pad=1, **init)  #
                self.skip_conv_det_5 = L.Convolution2D(512, 512, ksize=3, stride=1, pad=1, **init)  #

                # conv after skip connection

            if self.segmentation:
                self.deconv_seg_1 = L.Deconvolution2D(256, 256, ksize=3, stride=1, pad=0, **init)  ##  3*3
                ...
                self.deconv_seg_2 = L.Deconvolution2D(256, 256, ksize=3, stride=1, pad=0, **init)  ## 5*5
                ...
                self.deconv_seg_3 = L.Deconvolution2D(256, 256, ksize=4, stride=2, pad=1, **init)  ## 10*10

                self.deconv_conv_seg_3 = L.Convolution2D(256, 512, ksize=3, stride=1, pad=1, **init)  #
                self.skip_conv_seg_3 = L.Convolution2D(512, 512, ksize=3, stride=1, pad=1, **init)  #

                self.deconv_seg_4 = L.Deconvolution2D(512, 256, ksize=3, stride=2, pad=1, **init)  ##  19*19

                self.deconv_conv_seg_4 = L.Convolution2D(256, 1024, ksize=3, stride=1, pad=1, **init)  #
                self.skip_conv_seg_4 = L.Convolution2D(1024, 1024, ksize=3, stride=1, pad=1, **init)  #

                self.deconv_seg_5 = L.Deconvolution2D(1024, 256, ksize=4, stride=2, pad=1, **init)  ## 38*38

                self.deconv_conv_seg_5 = L.Convolution2D(256, 512, ksize=3, stride=1, pad=1, **init)  #
                self.skip_conv_seg_5 = L.Convolution2D(512, 512, ksize=3, stride=1, pad=1, **init)  #
    ...
    def __call__(self, x):
        ...
        stages = super(Multi_task_VGG16Extractor300, self).__call__(x)
        if self.attention:
...
class Multi_task_300(SSD):
    ...
    def __init__(self, n_fg_class=None, pretrained_model='imagenet', detection=True, segmentation=False,
        ...
        super(Multi_task_300, self).__init__(
            extractor=Multi_task_VGG16Extractor300(detection=detection, segmentation=segmentation, attention=attention),
            multibox=Multibox(
        ...
        with self.init_scope():
            self.multimask = Multimask(n_class=param['n_fg_class'] + 1, scale_num=6)
        self.detection = detection
        self.segmentation = segmentation
    ...
    def to_gpu(self, device=None):
        super(Multi_task_300, self).to_gpu(device)
